<div class="webinar-card webinar-list webinar-list-2 d-flex mt-30">
    ...
    <!-- Price Box -->
    <div class="webinar-price-box d-flex flex-column align-items-end text-right">
        @if(!empty($isRewardCourses) and !empty($webinar->points))
            <span style="color: #ffc107; font-weight: 600;">{{ $webinar->points }} {{ trans('update.points') }}</span>
        @elseif(!empty($webinar->price) and $webinar->price > 0)
            @if($webinar->bestTicket() < $webinar->price)
                <div>
                    <span class="real" style="color: #007bff; font-weight: bold; font-size: 1.1rem;">{{ handlePrice($webinar->bestTicket(), true, true, false, null, true) }}</span>
                    <span class="off" style="color: #6c757d; text-decoration: line-through; margin-left: 8px; font-size: 0.95rem;">{{ handlePrice($webinar->price, true, true, false, null, true) }}</span>
                </div>
                
                
            @else
                <span class="real" style="color: #007bff; font-weight: bold; font-size: 1.1rem;">{{ handlePrice($webinar->price, true, true, false, null, true) }}</span>
            @endif
        @else
            <span style="background-color: #28a745; color: white; padding: 5px 10px; border-radius: 4px; font-weight: bold;">{{ trans('public.free') }}</span>
        @endif
    </div>
        ...
        <br>
                       <!-- Button -->
    <a href="{{ $webinar->getUrl() }}" style="background-color: #28a745; color: white; padding: 5px 10px; border-radius: 4px; font-weight: bold; text-align: center; display: inline-block; min-width: 100px;">
    View Course
</a>
    </div>
</div>

        ...
        <div class="learning-page-progress-card d-flex flex-column">
            ...
            <div class="d-flex align-items-center">
                <div class="progress course-progress d-flex align-items-center flex-grow-1 bg-white border border-gray200 rounded-sm shadow-none mt-5">
                    <span id="courseProgressBar" class="progress-bar rounded-sm bg-warning" style="width: {{ $percent }}%"></span>
                
                </div>

                <span id="courseProgressText" class="ml-10 font-weight-500 font-14 text-gray">{{ $percent }}% {{ trans('update.learnt') }}</span>
            </div>
        </div>
    </div>
...
@push('scripts_bottom')
    <script src="{{ asset('assets/learning_page/liveProgressTracking.js') }}"></script>
@endpush

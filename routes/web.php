...
Route::get('health', "\Spatie\Health\Http\Controllers\HealthCheckResultsController@__invoke")
    ->middleware('auth');
...
Route::group(['namespace' => 'Web', 'middleware' => ['check_mobile_app', 'impersonate', 'share', 'check_maintenance', 'check_restriction']], function () {
    ...
    Route::get('/', 'HomeController@index')->name('home');//renamed the route
    ...
    Route::group(['prefix' => 'course'], function () {
        ...
        Route::get('/course/{id}/getProgressss', 'WebinarController@getProgressAjax')
            ->name('course.getProgress');
        ...
        Route::group(['middleware' => 'web.auth'], function () {
            ...
            Route::get('/assignment/{assignmentId}/history/{historyId}/message/{messageId}/downloadAttach', 'AssignmentHistoryController@downloadAttach');
             Route::post('/learning/', 'WebinarController@getProgressAjax')
             ->name('learning.mark.item.completed');
    ...
    Route::group(['middleware' => 'web.auth'], function () {
        ...
        Route::group(['prefix' => 'users'], function () {
            ...
            Route::post('/password-change', 'UserController@changePassword')->name('changedefaultpassword');
        });
...
Route::group(['namespace' => 'Auth', 'middleware' => ['check_mobile_app','share', 'check_maintenance', 'check_restriction']], function () {
   ...
   // Route::get('/@{code}', 'ReferralController@referral');
   
   Route::get('/{code}', 'ReferralController@referral')
    ->where('code', '^(?!login$|logout$|register$|verification$|forget-password$|reset-password$|google$|facebook$|dashboard$|panel$|admin$|health$|my_api$|api_sessions$|course$|cart$|users$|meetings$|become-instructor$|certificate_validation$|marketing$|stripe$|blog$|contact$|instructors$|organizations$|load_more$|pages$|captcha$|newsletters$|jobs$|regions$|instructor-finder$|products$|reward-products$|bundles$|forums$|upcoming_courses$|installments$|waitlists$|gift$|forms$)[a-zA-Z0-9_-]+$');

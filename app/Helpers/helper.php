...
function handlePrice($price, $showCurrency = true, $format = true, $coursePagePrice = false, $user = null, $showTaxInPrice = false, $taxType = 'general')
{
    ...
    // Set decimal and separator based on user's currency settings
    $decimal = $userCurrencyItem->currency_decimal ?? 2; // Default to 2 decimal places
    $decimalSeparator = '.'; // Always use dot as decimal separator
    $thousandsSeparator = ','; // Always use comma as thousands separator

    // Convert price to user's currency
    $price = convertPriceToUserCurrency($price, $userCurrencyItem);

    // Handle tax and price display options
    if ($priceDisplay != 'only_price') {
        $tax = getFinancialSettings('tax') ?? 0;
...
/**
 * Format the price with the correct decimal and thousands separators.
 */
function handlePriceFormat($price, $decimals = 2, $decimalSeparator = '.', $thousandsSeparator = ',')
{
    if ($price and $price > 0) {
 ...
 */
// function addCurrencyToPrice($price, $userCurrencyItem)
// {
//     $currencySymbol = $userCurrencyItem->currency_symbol ?? 'N'; // Default to 'N' if no symbol is set
...
function addCurrencyToPrice($price, $userCurrencyItem = null)
{
    ...
    if (!empty($price)) {
        $currency = currencySign($userCurrencyItem->currency); // Get the currency symbol (e.g., 'N')
        $currencyPosition = $userCurrencyItem->currency_position; // Get the currency position (e.g., 'left')

        switch ($currencyPosition) {
            case 'left':
                $price = $currency . $price; // Example: N200,000.00
                break;

            case 'left_with_space':
                $price = $currency . ' ' . $price; // Example: N 200,000.00
                break;

            case 'right':
                $price = $price . $currency; // Example: 200,000.00N
                break;

            case 'right_with_space':
                $price = $price . ' ' . $currency; // Example: 200,000.00 N
                break;

            default:
                $price = $currency . $price; // Default to left position
        }
    }

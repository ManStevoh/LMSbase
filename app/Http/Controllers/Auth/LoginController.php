...
class LoginController extends Controller
{
    ...
    {
        return $request->get('type') == 'mobile' ? 'mobile' : 'email';
    }
    ...
    public function afterLogged(Request $request, $verify = false)
    {
        ...
        // ✅ UPDATED BLOCK: Handle unverified users
        if ($user->status != User::$active && !$verify) {
            $this->guard()->logout();
            $request->session()->flush();
            $request->session()->regenerate();

            $loginField = $this->getUsername($request);
            $loginValue = $this->getUsernameValue($request);

            $verificationController = new VerificationController();
            $checkConfirmed = $verificationController->checkConfirmed($user, $loginField, $loginValue);

            if ($checkConfirmed['status'] === 'send') {
                $verification = \App\Models\Verification::where($loginField, $loginValue)->latest()->first();
                if ($verification) {
                    session()->put('verificationId', $verification->id);
                }

                return redirect('/verification')->with('resent', true);

...
class SettingsController extends Controller
{
    ...
    public function page(Request $request, $page)
    {
        ...
        if ($page == 'financial') {
            $paymentChannels = PaymentChannel::orderBy('created_at', 'desc')->paginate(50);
    ...
    public function storeAppSettings(Request $request)
    {
        ...
        $data = $request->all();
        $locale = $request->get('locale', Setting::$defaultSettingsLocale);
        $setting_key = $data['app_setting'];
        $values = [];

        $settings = Setting::where('name', Setting::$appSettings)->first();

        if (!empty($settings) and !empty($settings->value)) {
            $values = json_decode($settings->value);
            
            if ($setting_key !== 'app_setting') {
        ...
        cache()->forget('settings.' . Setting::$appSettings);

        return redirect(getAdminPanelUrl() . '/settings/general');
    }


    public function deleteAppSettings($social_key, $locale = null)
    {
        $this->authorize('admin_settings_general');
        $settings = Setting::where('name', Setting::$appSettings)->first();
    ...
    public function editAppSettings($social_key)
    {
        removeContentLocale();

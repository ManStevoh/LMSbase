...
class SupportsController extends Controller
{
    ...
    //             // Optionally search in this table directly
    ...
    //     if (!empty($email)) {
    //         $userIdsByEmail = \App\User::where('email', 'like', "%$email%")
    //             ->pluck('id')
    //             ->toArray();

    //         if (!empty($userIdsByEmail)) {
    //             $query->whereIn('user_id', $userIdsByEmail);
    //         } else {
    //             // Optional: return no results if email not matched
    //             $query->whereRaw('0 = 1');
    //         }
    //     }
    ...
    private function handleFilters($request, $query)
    {
        ...
        // If title is provided but search is not, use title as search
        ...
        if (!empty($search)) {
            $query->where(function ($query) use ($search) {
                // Search in the supports table - only use columns that actually exist
                $query->where('title', 'like', "%$search%");

                // If you have a description column, uncomment this line
                // ->orWhere('description', 'like', "%$search%");

                // Search in the user's names
                $query->orWhereHas('user', function ($userQuery) use ($search) {
                    $userQuery->where(function ($q) use ($search) {
                        $q->where('full_name', 'like', "%$search%")
                          ->orWhere('middle_name', 'like', "%$search%")
            ...
            // Fallback if search is not used but title is (for backward compatibility)
            $query->where('title', 'like', "%$title%");
        }
        ...
        // Rest of the filter code remains unchanged
        if (!empty($from) and !empty($to)) {
            $from = strtotime($from);
            $to = strtotime($to);

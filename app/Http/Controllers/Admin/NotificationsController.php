...
public function posted()
{
    ...
    // Add user search method for AJAX
    public function search(Request $request)
    {
        ...
        // Search for users matching the term in name, email, or mobile
        $users = User::where('full_name', 'LIKE', "%$term%")
    ...
    public function store(Request $request)
    {
        ...
        $this->validate($request, [
            ...
            // Add validation for user_ids array
        ...
        // Handle single user email notification
        if (!empty($user_id)) {
        ...
        // Handle selected_users type - NEW FEATURE
        if ($data['type'] === 'selected_users' && !empty($data['user_ids'])) {
            // Attach selected users to notification
            foreach ($data['user_ids'] as $selectedUserId) {
                $notification->users()->attach($selectedUserId, [
                    'created_at' => now(),
                    'updated_at' => now()
                ]);

                // Send email to each selected user
                $selectedUser = \App\User::where('id', $selectedUserId)->first();
                if (!empty($selectedUser) && !empty($selectedUser->email)) {
                    try {
                        \Mail::to($selectedUser->email)->send(new SendNotifications(['title' => $data['title'], 'message' => $data['message']]));
                    } catch (\Exception $exception) {
                        // Log exception
                    }
                }
    ...
    private function handleFirebaseMessages($data, $user_id, $group_id, $webinar_id)
    {
        ...
        // Add handler for selected_users type - NEW FEATURE
        if ($data["type"] === "selected_users") {
            if (empty($data['user_ids']) || !is_array($data['user_ids'])) {
                return true;
            }

            $fcmTokensQuery->whereIn('user_id', $data['user_ids']);
        }
    ...
    public function edit($id)
    {
        ...
        $notification = Notification::where('id', $id)
            ->with([
                ...
                'group',
                // Add relationship to users for selected_users type
                'users' => function ($query) {
    ...
    public function update(Request $request, $id)
    {
        ...
        // Handle selected_users type for updates - NEW FEATURE
        if ($data['type'] === 'selected_users') {
            // First detach all existing users
            $notification->users()->detach();

            // Then attach the new selection
    ...
    public function delete($id)
    {
        ...
        // Remove associated user selections - NEW FEATURE
        if ($notification->type === 'selected_users') {
            $notification->users()->detach();
        }

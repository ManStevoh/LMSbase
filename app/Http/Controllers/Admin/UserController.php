...
class UserController extends Controller
{
    ...
    public function studentBulkImport(Request $request)
    {
        ...
        try {
            Excel::import(new StudentBulkImport(), $request->file('file'));
            return response()->json(['message' => 'Students uploaded successfully and notification emails queued.'], 200);
        } catch (\Exception $e) {
...
public function students(Request $request, $is_export_excel = false)
{
    ...
    $inactiveStudents = deepClone($query)->where('status', 'inactive')->count();
...
{
    // Check what exact state value is being received
    \Log::info("Requested state: " . $state);

    $lgas = User::where('state', $state)
        ->whereNotNull('lga')
        ->distinct()
        ->pluck('lga');

    \Log::info("LGAs found: " . $lgas->toJson());
...
public function search(Request $request)
{
    ...
    //                         $query->orWhere('full_name', 'like', "%$part%")
    ...
    //                 // This helps when someone searches for "John Doe"
    //                 $query->orWhereRaw("CONCAT(full_name, ' ', COALESCE(middle_name, ''), ' ', COALESCE(last_name, '')) LIKE ?", ["%$full_name%"]);
    //                 $query->orWhereRaw("CONCAT(full_name, ' ', COALESCE(last_name, '')) LIKE ?", ["%$full_name%"]);
    //             }
    //         });
    //     }
    ...
    private function filters($query, $request)
{
        ...
        $query->where(function ($query) use ($full_name) {
            $query->where(function ($q) use ($full_name) {
                $q->where('full_name', 'like', "%$full_name%")
                    ->orWhereRaw("CONCAT(full_name, ' ', COALESCE(middle_name, ''), ' ', COALESCE(last_name, '')) LIKE ?", ["%$full_name%"])
                    ->orWhereRaw("CONCAT(full_name, ' ', COALESCE(last_name, '')) LIKE ?", ["%$full_name%"]);
            });
            ...
            if (count($nameParts) > 1) {
                ...
                foreach ($nameParts as $part) {
                    if (strlen($part) > 1) {
                        $scoreQuery .= "WHEN full_name LIKE ? THEN 3 ";
                        $bindParams[] = "%$part%";

                        $scoreQuery .= "WHEN middle_name LIKE ? THEN 2 ";
                        $bindParams[] = "%$part%";

                        $scoreQuery .= "WHEN last_name LIKE ? THEN 2 ";
                        $bindParams[] = "%$part%";
                    }
                }

                $scoreQuery .= "ELSE 0 END) as name_match_score";

                $query->orWhere(function ($q) use ($nameParts, $scoreQuery, $bindParams) {
                    foreach ($nameParts as $part) {
                        if (strlen($part) > 1) {
                    ...
                    $q->selectRaw($scoreQuery, $bindParams);
                });

                $query->orderByRaw('COALESCE(name_match_score, 0) DESC');
            }
        });
    }
    ...
    if (!empty($status)) {
        switch ($status) {
            case 'active_verified':
                $query->where('status', 'active')->where('verified', true);
                break;
            case 'active_notVerified':
                $query->where('status', 'active')->where('verified', false);
                break;
            case 'inactive':
                $query->where('status', 'inactive');
                break;
            case 'ban':
                $query->where('ban', true)->whereNotNull('ban_end_at')->where('ban_end_at', '>', time());
                break;
            case 'pending':
    ...
    public function bulk()
    {
        ...
        return view('admin.users.bulk', $data);
    }
    ...
    public function importBulk(Request $request)
    {
        ...
        while (!feof($handle)) {
            ...
            [$duplicates, $emails] = $this->getchunkdata($chunkdata, $role_id, $role, $status);
            $duplicateUsers = array_merge($duplicateUsers, $duplicates);
            $emailData = array_merge($emailData, $emails);
        }
        ...
        if (!empty($duplicateUsers)) {
            return back()->withErrors(['duplicate_users' => 'The following emails already exist: ' . implode(', ', $duplicateUsers)]);
        }

        return back()->with('success', 'Users imported and notified successfully!');
    }


    //     public function getchunkdata($chunkdata,$role_id,$role,$status)
    // {
    ...
    //         $employee->mobile = $phoneNumber;
    //         $employee->verified = true;
    //         $employee->status = $status;
    //         $employee->password = User::generatePassword($full_name);
    //         $employee->created_at = time();
    //         $employee->save();

    //         // dispatch(new StudentBulkUploadEmailNotification(
    //         //     $email,
    //         //     $email['name'],
    //         //     $email['login_url'],
    //         //     $email['password']
    ...
    public function getchunkdata($chunkdata, $role_id, $role, $status)
    {
        ...
        foreach ($chunkdata as $column) {
            $full_name = $column[0] ?? null;
            $middle_name = $column[1] ?? null;
            $last_name = $column[2] ?? null;
            $email = $column[3] ?? null;
            $phoneNumber = $column[4] ?? null;
        ...
        return [$duplicateUsers, $emailData];
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $this->validate($request, [
            'full_name' => 'required|min:3|max:128',
            'middle_name' => 'nullable|min:3|max:128',
        ...
        $user->middle_name = !empty($data['middle_name']) ? $data['middle_name'] : null;
  ...
  //saving the UPDATED AFFILIATE CODE
        if (!empty($data['affiliate_code'])) {
    // Check if the code already exists for a different user
    $existingCode = AffiliateCode::where('code', $data['affiliate_code'])
        ->where('user_id', '!=', $user->id)
        ->first();

    if ($existingCode) {
        return back()->withErrors(['affiliate_code' => 'Affiliate code must be unique.'])->withInput();
    }

    // Save or update affiliate code
    AffiliateCode::updateOrCreate(
        ['user_id' => $user->id],

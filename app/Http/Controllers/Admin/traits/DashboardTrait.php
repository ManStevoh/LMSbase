...
trait DashboardTrait
{
    ...
    public function getEnrollmentsChart($type = 'month_of_year')
    {
        $labels = [];
        $data = [];

        $query = Sale::whereNull('refund_at');
        ...
        if ($type == 'day_of_month') {
            ...
            for ($day = 1; $day <= 31; $day++) {
                ...
                $amount = $this->salesCounts(deepClone($query), $startDay, $endDay);
                $data[] = round($amount, 2);
            }
        } elseif ($type == 'month_of_year') {
            for ($month = 1; $month <= 12; $month++) {
                ...
                $amount = $this->salesCounts(deepClone($query), $start_date, $end_date);
                $data[] = round($amount, 2);
            }
        }
    ...
    public function getNetCertChart($type = 'month_of_year')
    {
        $labels = [];
        $data = [];
        ...
        if ($type == 'day_of_month') {
            ...
            for ($day = 1; $day <= 31; $day++) {
                ...
                $amount = $this->certCounts(deepClone($query), $startDay, $endDay);
                $data[] = round($amount, 2);
            }
        } elseif ($type == 'month_of_year') {
            for ($month = 1; $month <= 12; $month++) {
                ...
                $amount = $this->certCounts(deepClone($query), $start_date, $end_date);
                $data[] = round($amount, 2);
            }
        }
    ...
    private function salesCounts($query, $start, $end)
    {
        $count = deepClone($query)->whereBetween('created_at', [$start, $end])
            ->count();

        return $count;
    }

    private function certCounts($query, $start, $end)
    {
        $count = deepClone($query)->whereBetween('created_at', [$start, $end])
            ->count();
    ...
    public function getMostRecentEnrollments()
    {
        $enrollments = Sale::with('buyer')->whereNull('refund_at')
            ->orderBy('created_at', 'desc')
            ->limit(10)
            ->get();
        return $enrollments;
    ...
    public function getTenMostActiveStudents()
    {
        return User::where('status', 'active')
            ->join('sales', 'users.id', '=', 'sales.buyer_id')
            ->leftJoin('certificates', 'users.id', '=', 'certificates.student_id')
            ->select(
                'users.*',
                'sales.buyer_id',
                DB::raw('count(DISTINCT sales.meeting_id) as reserved_appointments'),

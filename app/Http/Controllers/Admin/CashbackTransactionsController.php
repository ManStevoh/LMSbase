...
class CashbackTransactionsController extends Controller
{
    ...
    private function handleFilters(Request $request, $query)
    {
        ...
        // Optional: title filter logic placeholder
        if (!empty($title)) {
        ...
        // General search with advanced user logic
        if (!empty($search)) {
    ...
    public function history(Request $request)
    {
        ...
        $query = Accounting::query()
            ...
            ->select(
                'accounting.*',
                DB::raw('sum(accounting.amount) as total_cashback'),
                DB::raw('max(accounting.created_at) as last_cashback'),
                DB::raw('sum(order_items.total_amount) as purchase_amount')
            )
            ->where('is_cashback', true)
            ->where('system', false)
            ->where('type', Accounting::$addiction)
